...
export default async function handler(req, res) {
  try {
    ...
    // Check if path is in query params (from rewrite rule with catch-all pattern)
    if (req.query && req.query.path) {
      const pathParam = req.query.path;
      const pathArray = Array.isArray(pathParam) ? pathParam : (pathParam ? [pathParam] : []);
      path = '/' + pathArray.join('/');
      
      // Get query string from req.query (excluding the 'path' param)
      ...
      // Fallback: Extract from req.url directly
